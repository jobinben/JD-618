...
function createSuperAssistUser() {
  return new Promise(resolve => {
    ...
    $.get({ url: 'https://raw.githubusercontent.com/inoyna11/jd28/master/shareCodes/cfda.json' }, async (err, resp, _data) => {
      try {
...
function joinGroup() {
  return new Promise( async (resolve) => {
    $.get({ url: 'https://raw.githubusercontent.com/inoyna11/jd28/master/shareCodes/cfdb.json' }, (err, resp, _data) => {
      try {
        const { data = {} } = JSON.parse(_data);
