...
function getcode() {
    return new Promise(async (resolve) => {
        let options = taskUrl("openRedEnvelopeInteract", `{"linkId":"${$.linkid}","redEnvelopeId":"${rid}","inviter":"${inviter}","helpType":"1"}`)
        $.get(options, async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`);
...
function taskUrl(function_id, body) {
    return {
        ...
        headers: {
            ...
            "Host": "api.m.jd.com",
            "Referer": "https://618redpacket.jd.com/?activityId=DA4SkG7NXupA9sksI00L0g&channel=wjicon&sid=3bc8ea225c0a37a5608cf327e87d6aew&un_area=22_2033_2037_7245",
            "Cookie": cookie,
